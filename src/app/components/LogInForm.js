...
export default function LogInForm({ isHidden, closeModal }) {
    ...
    return (
        <div>
            <LogIn isHidden={hidden} closeModal={closeModal} switchToSignUp={toggleHidden} />
            <SignUp isHidden={!hidden} closeModal={closeModal} switchToLogIn={toggleHidden} />
        </div>
    )
};
    ...
    <div>
      ...
      <div className="fixed inset-0 flex justify-center items-center z-50">
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full space-y-4">
          ...
          <p>Don't have an account? <button className='text-blue-600 underline' onClick={switchToSignUp}>Sign Up!</button> </p>
        </div>
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div className="fixed inset-0 flex justify-center items-center z-50">
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full space-y-4">
          ...
          <p>Already have an account? <button className='text-blue-600 underline' onClick={switchToLogIn}>Log In!</button> </p>
        </div>
      </div>
    </div>
  );
